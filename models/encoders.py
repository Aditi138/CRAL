...
class Typology_Feature_Encoder(Encoder):
    def __init__(self, model, feature_input_dim, args, id_to_word):
        Encoder.__init__(self)
        self.W, self.b = {},{}
        for lang in args.langs.split("/"):
            lang = "<" + lang + ">"
            self.W[lang]  = model.add_parameters((args.typology_feature_dim, feature_input_dim))
            self.b[lang] = model.add_parameters(args.typology_feature_dim)
            self.b[lang].zero()
        # self.W = model.add_parameters((args.typology_feature_dim, feature_input_dim))
        # self.b = model.add_parameters(args.typology_feature_dim)
        # self.b.zero()
        self.dim = args.typology_feature_dim
        self.id_to_word = id_to_word


    def get_features(self, input_feats, input_langs, pre_computed_features, batch_size, input_dim):
        # W = dy.parameter(self.W)
        # b = dy.parameter(self.b)
        encoded_typological_feature = []
        for emb, langid in zip(pre_computed_features, input_langs):
            lang = self.id_to_word[langid[0]]
            W = dy.parameter(self.W[lang])
            b = dy.parameter(self.b[lang])
            encoded_typological_feature.append(dy.tanh(dy.affine_transform([b,W,emb])))


        #encoded_typological_feature = [dy.tanh(dy.affine_transform([b, W, emb])) for emb in pre_computed_features]
        encoded_typological_feature = dy.reshape(dy.concatenate_cols(encoded_typological_feature), (self.dim,),
                                                 batch_size=batch_size)

        factorized_output_emb = []
        # debug = np.array([i for i in range(400)])
        # debug = dy.reshape(dy.inputTensor(np.concatenate([debug, debug])), (400,), batch_size=batch_size)
        for wif in input_feats:
            extend_wif = dy.transpose(dy.concatenate_cols([wif for _ in range(self.dim)]))
            feature_emb = dy.cmult(extend_wif, encoded_typological_feature)
            factorized_mat = dy.reshape(feature_emb, (self.dim * input_dim,), batch_size=batch_size)
            factorized_output_emb.append(factorized_mat)


        return factorized_output_emb
...
class BiRNN_Encoder(Encoder):
    ...
    def encode(self, input_seqs, training=True, char=False, model_char=False):
        if char:
            return self.encode_word(input_seqs, training=training, model_char=model_char)
        else:
    ...
    def encode_seq(self, input_seqs, training=True, char=False):
        ...
        if char:
            return dy.concatenate([fwd_vectors[-1], bwd_vectors[0]]), fwd_vectors, bwd_vectors
        ...
        return birnn_outputs, fwd_vectors, bwd_vectors

    def encode_word(self, input_seqs, training=True, model_char=False):
        ...
        for sent in input_seqs:
            ...
            for w in sent:
                if model_char:
                    w_emb,_,_ = self.encode_seq(w, training=training, char=True)
                else:
                    w_emb,_,_ = self.encode_seq([w], training=training, char=True)
                sent_emb.append(w_emb)
            sents_embs.append(sent_emb)
    ...
    def encode_char(self, input_seqs, training=True):
        # embedding dropout rate is 0.0, because we dropout at the later stage of RNN
        sents_embs = []
        ...
        for sent in input_seqs:
            sent_emb = []
            for w in sent:
                w_emb,_,_ = self.encode_seq([w], training=training)
                sent_emb.append(w_emb)
            sents_embs.append(sent_emb)
        return sents_embs