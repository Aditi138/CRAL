...
def evaluate(data_loader, path, model, model_name, type="dev"):
    ...
    sents, char_sents, tgt_tags, discrete_features, bc_feats, _ = data_loader.get_data_set_morph_monolithic(path,
                                                                                                            args.lang,
                                                                                                            source="dev")
    #print(len(sents[0]), len(char_sents[0]), len(tgt_tags[0]), len(pos_tags[0]))
    ...
    model.classifier.sent_index = 0
    model.tokens = []
    model.to_index = 0
    for sent, char_sent, discrete_feature, bc_feat, tgt_tag in zip(sents, char_sents, discrete_features, bc_feats, tgt_tags):
        dy.renew_cg()
        sent, char_sent, discrete_feature, bc_feat, b_lang = [sent], [char_sent], [discrete_feature], [bc_feat], [ [langid for _ in range(len(sent))]]
        if args.al_mode == "qbc":
            best_scores, best_paths, fwd_best_paths, bwd_best_paths, futback_best_paths = model.eval_cvt(sent,
                                                                                                         char_sent,
                                                                                                         training=False,
                                                                                                         type=type,
                                                                                                         tgt_tags=tgt_tags)

        elif args.al_mode == "entropy": #UNS
            best_scores, best_paths = model.eval(sent, char_sent, discrete_feature, bc_feat, langs=b_lang,
                                                 training=False, type=type, tgt_tags=tgt_tags)
            fwd_best_paths, bwd_best_paths, futback_best_paths = best_paths, best_paths, best_paths


        else: #Propoe method CRAL
        ...
        sent_score = 0.0
        assert len(best_paths) == len(tgt_tag)
        for token_num, pred_tag_feat in enumerate(best_paths):
            tag_name = data_loader.id2tags[pred_tag_feat]
            one_prediction.append([tag_name])
...
def main(args):
    ...
    monolingual_data_loader = None

    if args.multilingual:
        sents, char_sents, tgt_tags, discrete_features, bc_features, known_tags, langs, typological_features = [], [], [], [], [], [], [], []
        ...
        for lang in all_langs:
            ...
            lang_sents, lang_char_sents, lang_tgt_tags, lang_discrete_features, lang_bc_features, lang_known_tags = pos_data_loader.get_data_set_morph_monolithic(
                train_path, lang)

            sents += lang_sents
            char_sents += lang_char_sents
            tgt_tags += lang_tgt_tags
            known_tags += lang_known_tags
    ...
    if not args.test_conll:
        data_test = pos_data_loader.get_lr_test(args.test_path, args.lang)
    # print pos_data_loader.char_to_id
    ...
    model = BiRNN_ATTN_CRF_model_CVT(args, pos_data_loader, monolingual_data_loader)
    ...
    if args.fineTune:
        ...
        model.load()
        if args.special_lr:
             inital_lr = args.init_lr + (0.001 * len(sents) / args.k )
    ...
    while epoch <= args.tot_epochs:
        ...
        unlabeled_sents, unlabeled_char_sents, unlabeled_tgt_tags, _, _, unlabeled_known_tags = pos_data_loader.get_data_set_morph_monolithic(
            args.unlabeled_path, args.lang)
        unlabeled_langs = ["<" + args.lang + ">" for _ in range(len(unlabeled_sents))]
        unlabeled_sent_index = [i for i in range(len(unlabeled_sents))]
        unlabeled_batches = make_bucket_batches(
            zip(unlabeled_sents, unlabeled_char_sents, unlabeled_langs, unlabeled_tgt_tags, unlabeled_known_tags,
                unlabeled_sent_index), None, None, batch_size)

        batches = make_bucket_batches(
            zip(sents, char_sents, tgt_tags, known_tags, discrete_features, bc_features, langs, sent_index), None,
            None,
            batch_size)
        ...
        for unlabeled_b_sents, unlabeled_b_char_sents, unlabeled_b_langs, _,  unlabeled_tgt_tags, unlabeled_known_tags in unlabeled_batches:
            ...
            for _ in range(len(unlabeled_b_sents)):
            ...
            b_sents, b_char_sents, b_tgt_tags, b_known_tags, b_feats, b_bc_feats, b_langs, _ = batches[
                supervised_batch_index]
            dy.renew_cg()
            ...
            loss = model.cal_loss(b_sents, b_char_sents, b_tgt_tags, b_feats, b_bc_feats, b_known_tags, langs=lang_batch, lm_batch=None,
                                  training=True)
            loss_val = loss.value()
            ...
            if args.print_epoch and epoch in [100, 101, 200, 201, 300, 301]:
                print("Printing at ", epoch)
                _, _, _, _, _, _, _, _, _ = evaluate(pos_data_loader, args.test_path, model,
                                                                     args.model_name + "_epoch_" + str(epoch) + "_", type="test",task=args.task)
            ...
            if updates % valid_freq == 0:
                acc, precision, recall, f1, _, _,_ ,_,_= evaluate(pos_data_loader, args.dev_path, model, args.model_name, task=args.task)
                print(acc, f1)
...
def finalEval(args, dev_pos_tags_idx, f1, model, pos_data_loader, test_pos_tags_idx):
...
def test_single_model(args):
    pos_data_loader = NER_DataLoader(args)
...
args = init_config()
from models.model_builder_cvt_softmax import *
